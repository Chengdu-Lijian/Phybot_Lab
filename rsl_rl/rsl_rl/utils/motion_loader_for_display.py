...
class AMPLoaderDisplay:
    ...
    def __init__(
        ...
        if len(motion_files) == 0:
            raise FileNotFoundError("No display motion files found. Please check `amp_motion_files_display` paths.")
    ...
    def _expand_motion_files(motion_files):
        ...
        for motion_file in motion_files:
            ...
            if motion_path.is_dir():
                expanded.extend(str(path) for path in sorted(motion_path.glob("*.json")))
                expanded.extend(str(path) for path in sorted(motion_path.glob("*.txt")))
                continue
            if motion_path.exists():
                expanded.append(str(motion_path))
                continue

            fallback = AMPLoaderDisplay._resolve_missing_motion_file(motion_path)
