...
class AMPLoader:
    ...
    def __init__(
        ...
        motion_files = self._expand_motion_files(motion_files)
        if len(motion_files) == 0:
            raise FileNotFoundError("No AMP motion files found. Please check `amp_motion_files` paths.")
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file) as f:
                ...
                if motion_data.ndim != 2:
                    raise ValueError(f"Motion file {motion_file} has invalid frame shape: {motion_data.shape}")

                if self._observation_dim is None:
                    self._observation_dim = motion_data.shape[1]
                elif motion_data.shape[1] != self._observation_dim:
                    raise ValueError(
                        "AMP motion files must share the same frame dimension. "
                        f"Got {motion_data.shape[1]} in {motion_file}, expected {self._observation_dim}."
                    )

                traj_tensor = torch.tensor(motion_data, dtype=torch.float32, device=device)
                self.trajectories.append(traj_tensor)
                self.trajectories_full.append(traj_tensor)
                self.trajectory_idxs.append(i)
                self.trajectory_weights.append(float(motion_json.get("MotionWeight", 1.0)))
                frame_duration = float(motion_json.get("FrameDuration", self.time_between_frames))
    ...
    @staticmethod
    def _expand_motion_files(motion_files):
        if isinstance(motion_files, str):
            motion_files = [motion_files]

        expanded = []
        for motion_file in motion_files:
            if any(ch in motion_file for ch in ["*", "?", "["]):
                expanded.extend(sorted(glob.glob(motion_file)))
            elif os.path.isdir(motion_file):
                expanded.extend(sorted(glob.glob(os.path.join(motion_file, "*.json"))))
                expanded.extend(sorted(glob.glob(os.path.join(motion_file, "*.txt"))))
            elif os.path.exists(motion_file):
                expanded.append(motion_file)
            else:
                expanded.extend(AMPLoader._resolve_missing_motion_file(Path(motion_file)))
        return list(dict.fromkeys(expanded))

    @staticmethod
    def _resolve_missing_motion_file(motion_path: Path) -> list[str]:
        parent = motion_path.parent
        if not parent.exists():
            return []

        stem = motion_path.stem
        prefix_matches = [str(path) for path in sorted(parent.glob(f"{stem}*.json"))]
        prefix_matches += [str(path) for path in sorted(parent.glob(f"{stem}*.txt"))]
        if prefix_matches:
            print(
                f"[WARN] Motion file not found: {motion_path}. "
                f"Using files that match prefix '{stem}' in {parent}."
            )
            return prefix_matches

        fallback_matches = [str(path) for path in sorted(parent.glob("*.json"))]
        fallback_matches += [str(path) for path in sorted(parent.glob("*.txt"))]
        if fallback_matches:
            print(
                f"[WARN] Motion file not found: {motion_path}. "
                f"Using all motion files available in {parent}."
            )
