...
from legged_lab.envs.phybot_c2.run_cfg import PhybotC2RunFlatEnvCfg
from legged_lab.envs.phybot_c2.run_with_sensor_cfg import PhybotC2RunWithSensorFlatEnvCfg
from legged_lab.envs.phybot_c2.walk_cfg import PhybotC2WalkFlatEnvCfg
from legged_lab.envs.phybot_c2.walk_with_sensor_cfg import (
    PhybotC2WalkWithSensorFlatEnvCfg,
)
...
class PhybotC2(VecEnv):
    def __init__(
        self,
        cfg: (
            PhybotC2RunFlatEnvCfg
            | PhybotC2WalkFlatEnvCfg
            | PhybotC2WalkWithSensorFlatEnvCfg
            | PhybotC2RunWithSensorFlatEnvCfg
        ),
        headless,
    ):
        self.cfg: (
    ...
    def init_buffers(self):
        ...
        # Any contact on non-foot links triggers reset.
        contact_body_count = self.contact_sensor.data.net_forces_w.shape[1]
        non_feet_mask = torch.ones(contact_body_count, dtype=torch.bool, device=self.device)
        feet_body_ids = torch.as_tensor(self.feet_cfg.body_ids, dtype=torch.long, device=self.device)
        non_feet_mask[feet_body_ids] = False
        self.non_feet_body_ids = torch.nonzero(non_feet_mask, as_tuple=False).squeeze(-1)

        self.feet_body_ids, _ = self.robot.find_bodies(
            name_keys=["left_ankle_roll", "right_ankle_roll"], preserve_order=True
        )
        self.elbow_body_ids, _ = self.robot.find_bodies(
            name_keys=["left_elbow_pitch", "right_elbow_pitch"], preserve_order=True
        )
        ...
        self.ankle_joint_ids, _ = self.robot.find_joints(
            name_keys=["left_ankle_pitch", "right_ankle_pitch", "left_ankle_roll", "right_ankle_roll"],
            preserve_order=True,
        )
        amp_joint_names = [
            "left_hip_pitch",
        ...
        self.amp_joint_ids, _ = self.robot.find_joints(name_keys=amp_joint_names, preserve_order=True)
        ...
        self.use_gait_phase = bool(getattr(gait_cfg, "enable", False))
        ...
        self.phase_offset = torch.zeros(self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False)
        if self.use_gait_phase:
            self.gait_cycle[:] = self.cfg.gait.gait_cycle
    ...
    def compute_current_observations(self):
        ...
        # print('self.obs_scales.projected_gravity: ', self.obs_scales.projected_gravity)
        # print('self.obs_scales.commands: ', self.obs_scales.commands)
        # print('self.obs_scales.joint_pos: ', self.obs_scales.joint_pos)
        # print('self.obs_scales.joint_vel: ', self.obs_scales.joint_vel)
        # print('self.obs_scales.actions: ', self.obs_scales.actions)
    ...
    def check_reset(self):
        # 1) Height-based termination for collapsed poses.
        height_termination = self.robot.data.root_pos_w[:, 2] < 0.20

        reset_buf = height_termination
    ...
    def init_obs_buffer(self):
        if self.add_noise:
            ...
            idx += 3
            noise_vec[idx : idx + 3] = 0.0  # commands
            idx += 3
            ...
            idx += self.num_actions
            noise_vec[idx : idx + self.num_actions] = 0.0  # previous actions
            idx += self.num_actions
            if self.use_gait_phase:
                noise_vec[idx : idx + 6] = 0.0  # gait phase features
