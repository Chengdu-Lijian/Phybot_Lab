...
def track_lin_vel_xy_yaw_frame_exp(
    env: BaseEnv | PhybotC2, std: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
...
def track_lin_vel_xy_yaw_frame_l2(
    env: BaseEnv | PhybotC2, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Penalize xy linear-velocity tracking error in yaw frame (difference to command)."""
    asset: Articulation = env.scene[asset_cfg.name]
    vel_yaw = math_utils.quat_rotate_inverse(
        math_utils.yaw_quat(asset.data.root_quat_w), asset.data.root_lin_vel_w[:, :3]
    )
    return torch.sum(torch.square(env.command_generator.command[:, :2] - vel_yaw[:, :2]), dim=1)


def track_ang_vel_z_world_l2(
    env: BaseEnv | PhybotC2, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Penalize yaw angular-velocity tracking error in world frame (difference to command)."""
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.square(env.command_generator.command[:, 2] - asset.data.root_ang_vel_w[:, 2])


def lin_vel_z_l2(env: BaseEnv | PhybotC2, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.square(asset.data.root_lin_vel_b[:, 2])


def ang_vel_xy_l2(env: BaseEnv | PhybotC2, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.sum(torch.square(asset.data.root_ang_vel_b[:, :2]), dim=1)


def energy(env: BaseEnv | PhybotC2, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    asset: Articulation = env.scene[asset_cfg.name]
    reward = torch.norm(torch.abs(asset.data.applied_torque * asset.data.joint_vel), dim=-1)
    return reward


def joint_acc_l2(env: BaseEnv | PhybotC2, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.sum(torch.square(asset.data.joint_acc[:, asset_cfg.joint_ids]), dim=1)


def action_rate_l2(env: BaseEnv | PhybotC2) -> torch.Tensor:
    return torch.sum(
        torch.square(
...
def undesired_contacts(env: BaseEnv | PhybotC2, threshold: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
...
def fly(env: BaseEnv | PhybotC2, threshold: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    net_contact_forces = contact_sensor.data.net_forces_w_history
    is_contact = torch.max(torch.norm(net_contact_forces[:, :, sensor_cfg.body_ids], dim=-1), dim=1)[0] > threshold
...
def is_terminated(env: BaseEnv | PhybotC2) -> torch.Tensor:
    """Penalize terminated episodes that don't correspond to episodic timeouts."""
    return env.reset_buf * ~env.time_out_buf


def feet_air_time_positive_biped(
    env: BaseEnv | PhybotC2, threshold: float, sensor_cfg: SceneEntityCfg
) -> torch.Tensor:
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
...
def feet_slide(
    env: BaseEnv | PhybotC2, sensor_cfg: SceneEntityCfg, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
...
def body_force(
    env: BaseEnv | PhybotC2, sensor_cfg: SceneEntityCfg, threshold: float = 500, max_reward: float = 400
) -> torch.Tensor:
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
...
def joint_deviation_l1(env: BaseEnv | PhybotC2, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    asset: Articulation = env.scene[asset_cfg.name]
...
def feet_stumble(env: BaseEnv | PhybotC2, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    return torch.any(
...
def feet_too_near_humanoid(
    env: BaseEnv | PhybotC2, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"), threshold: float = 0.2
) -> torch.Tensor:
...
def ankle_torque(env: PhybotC2) -> torch.Tensor:
...
def gait_feet_frc_perio(env: PhybotC2, delta_t: float = 0.02) -> torch.Tensor:
...
def gait_feet_spd_perio(env: PhybotC2, delta_t: float = 0.02) -> torch.Tensor:
...
def gait_feet_frc_support_perio(env: PhybotC2, delta_t: float = 0.02) -> torch.Tensor:
